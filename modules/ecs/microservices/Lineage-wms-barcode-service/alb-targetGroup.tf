...
resource "aws_alb_target_group" "tf_alb_tg_lineage_wms_barcode_service" {
...
resource "aws_alb_listener_rule" "tf_alb_listener_rule_lineage_wms_barcode_service" {
  listener_arn = var.albListener
  priority     = var.albPathPriority
  action {
    type             = "forward"
    target_group_arn = aws_alb_target_group.tf_alb_tg_lineage_wms_barcode_service.arn
  }
  condition {
    path_pattern {
...
resource "aws_alb_listener_rule" "tf_alb_listener_rule_lineage_wms_barcode_service_https" {
